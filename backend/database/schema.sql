-- ===================================
-- ایجاد پایگاه داده ai_excell2form
-- DataSave Project Database Schema
-- ===================================
...
CREATE TABLE IF NOT EXISTS `ai_users` (
    ...
    CONSTRAINT `fk_users_created_by` FOREIGN KEY (`created_by`) REFERENCES `ai_users` (`id`) ON DELETE SET NULL,
    CONSTRAINT `fk_users_updated_by` FOREIGN KEY (`updated_by`) REFERENCES `ai_users` (`id`) ON DELETE SET NULL
...
-- Trigger برای بروزرسانی تاریخ تغییر رمز عبور
CREATE TRIGGER `tr_users_password_changed` 
...
-- Trigger برای ثبت لاگ تغییرات کاربران
CREATE TRIGGER `tr_users_audit_log` 
AFTER UPDATE ON `ai_users`
FOR EACH ROW
BEGIN
    INSERT INTO `ai_system_logs` (
        ...
        JSON_OBJECT(
            'changed_fields', JSON_ARRAY(
                ...
                CASE WHEN OLD.user_group_id != NEW.user_group_id THEN 'user_group_id' END
            )
        )
    );
END$$
...
-- ایجاد کاربر مخصوص برنامه (در صورت نیاز)
-- CREATE USER IF NOT EXISTS 'ai_app_user'@'localhost' IDENTIFIED BY 'SecurePassword123!';
-- GRANT SELECT, INSERT, UPDATE, DELETE ON ai_excell2form.* TO 'ai_app_user'@'localhost';
-- FLUSH PRIVILEGES;
...
-- بهینه‌سازی جداول
OPTIMIZE TABLE `ai_users`, `ai_user_groups`, `ai_user_sessions`, `ai_system_logs`, `ai_user_settings`;

-- نمایش اطلاعات جداول ایجاد شده
SHOW TABLE STATUS WHERE Name LIKE 'ai_%';